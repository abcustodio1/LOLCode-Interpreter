...
const ifelse = (tokens) => {
	...
	if(tokens[0].type === "IF Statement Start Keyword"){
		...
		// otherwise, throws error
			...
			error_prompt("Invalid Start of O RLY?")
			return ERROR;
		}
	}
		...
		//if NO WAI
			...
			// if not YA RLY, Throw error
				...
				error_prompt("Expected YA RLY Before NO WAI");
				return ERROR;
			}
		}
		...
		// if OIC
			...
			if(ifelse_stack[ifelse_tos] === "IF Statement Start Keyword"){
				error_prompt("Expected YA RLY Before OIC");
				return ERROR;
			}
		...
		// for statement calls
			...
			// if it is not under YA RLY or NO WAI, throw error
				...
				error_prompt("Expected YA RLY before " + tokens[0].lexeme);
				return ERROR;
			}
		}
...
const statement_block = (tokens) => {
	/*tokens = [];
	return FINISH;*/
	var statements = [ print_statement, operator, smoosh, assignment_statement, infinite_logical, input_statement];
	var result;
	for (let i = 0; i < statements.length; i++) {
      ...
      if (result === PASS && i === statements.length - 1) {
        error_prompt("Not Allowed inside Conditional Blocks");
        return ERROR;
      }
    }
...
const error_prompt = (message) => {
	document.getElementById("consoleArea").innerHTML += ("<br> <font color=\'red\'>" + message + "</font>");	
}