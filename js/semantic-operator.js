	...
	if(operators.includes(tokens[0].type)){
		...
		for(let i = 1; i < tokens.length; ++i){
			...
			if(an_flag){
				...
				// check if its literal or variable
					...
					if(tokens[i].type.includes("Identifier")){
						...
						if(index === -1){
							error_prompt("Cannot find Identifier " + tokens[i].lexeme);
							operation = [];
							operation_tos = -1;
							literal_stack = [];
							literal_tos = -1;
			...
			// otherwise, throw error
				...
				error_prompt("Insufficient Operands for " + operation[operation_tos].operator);
				operation = [];
				operation_tos = -1;
				literal_stack = [];
				literal_tos = -1;
		...
		if(operation.length === 1 && literal_stack.length === 0){
			error_prompt("Expected Literal/Variable/Operator after " + operation[operation_tos].operator);
			operation = [];
			operation_tos = -1;
			literal_stack = [];
			literal_tos = -1;
	...
	switch(operator){
		case "Addition Arithmetic Operator":
			...
			if(!isNaN(op1) && !isNaN(op2)){
				...
				error_prompt("SUM OF Detected Invalid operands");
				return ERROR;
			}
		case "Subtraction Arithmetic Operator":
			...
			if(!isNaN(op1) && !isNaN(op2)){
				...
				error_prompt("DIFF OF Detected Invalid operands");
				return ERROR;
			}
		case "Multiplication Arithmetic Operator":
			...
			if(!isNaN(op1) && !isNaN(op2)){
				...
				error_prompt("PRODUKT OF Detected Invalid operands");
				return ERROR;
			}
		case "Division Arithmetic Operator":
			...
			if(!isNaN(op1) && !isNaN(op2)){
				if(op2 !== 0){
					...
					error_prompt("Division by zero is undefined");
					return ERROR;
				}
			}
			else{
				error_prompt("QUOSHUNT OF Detected Invalid operands");
				return ERROR;
			}
		case "Modulo Arithmetic Operator":
			...
			if(!isNaN(op1) && !isNaN(op2)){
				...
				error_prompt("MOD OF Detected Invalid operands");
				return ERROR;
			}
		case "MAX Operator":
			op1 = parseFloat(first_operand);
			...
			if(!isNaN(op1) && !isNaN(op2)){
				...
				error_prompt("BIGGR OF Detected Invalid operands");
				return ERROR;
			}
		case "MIN Operator":
			op1 = parseFloat(first_operand);
			...
			if(!isNaN(op1) && !isNaN(op2)){
				...
				error_prompt("SMALLR OF Detected Invalid operands");
				return ERROR;
			}
		case "AND Logical Operator":
			if((first_operand === "WIN" || first_operand === "FAIL") && (second_operand === "WIN" || second_operand === "FAIL")){
				...
				error_prompt("BOTH OF Detected Invalid operands");
				return ERROR;
			}
		case "OR Logical Operator":
			if((first_operand === "WIN" || first_operand === "FAIL") && (second_operand === "WIN" || second_operand === "FAIL")){
				...
				error_prompt("EITHER OF Detected Invalid operands");
				return ERROR;
			}
		case "XOR Logical Operator":
			if((first_operand === "WIN" || first_operand === "FAIL") && (second_operand === "WIN" || second_operand === "FAIL")){
				...
				error_prompt("WON OF Detected Invalid operands");
				return ERROR;
			}
		...
		default:
			if(second_operand === "WIN" || second_operand === "FAIL"){
				...
				error_prompt("NOT OF Detected Invalid operand");
				return ERROR;
			}
	}
}
