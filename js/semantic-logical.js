...
const semantic_logical = (tokens) => {
	...
	if(operators.includes(tokens[0].type)){
		...
		for(let i = 1; i < tokens.length; ++i){
			...
			// otherwise, throw error
				...
				error_prompt("Insufficient Operands for " + logic_stack[logic_tos].operator);
				logic_stack = [];
				logic_tos = -1;
				boolean_stack = [];
				boolean_tos = -1;
		...
		if(logic_stack.length === 1 && boolean_stack.length === 0){
			error_prompt("Expected Literal/Variable/Operator after " + logic_stack[logic_tos].operator);
			logic_stack = [];
			logic_tos = -1;
			boolean_stack = [];
			boolean_tos = -1;
	...
	// check if only literal or variable
		...
		if(tokens[0].type.includes("Identifier")){
			...
			if(index === -1){
				error_prompt("Cannot find Identifier " + tokens[0].lexeme);
				return {status: ERROR, next: 0};
			}
			else{
				if(symbol_table[index].value === "WIN" || symbol_table[index].value === "FAIL"){
					...
					error_prompt(symbol_table[index].value + " is not of type TROOF!");
					return {status: ERROR, next: 0};
				}
			}						
		}
			...
			if(tokens[0].lexeme === "WIN" || tokens[0].lexeme === "FAIL"){
				...
				error_prompt(tokens[0].lexeme + " is not of type TROOF!");
				return {status: ERROR, next: 0};
			}
		}
		// display("Pushed into Boolean-Stack: " + boolean_stack[boolean_tos]);
