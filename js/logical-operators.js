...
const syntax_logical = (tokens) => {
	...
	if(operators.includes(tokens[0].type)){
		...
		++log_tos;
		// error_prompt("Pushed into Log-Stack: " + tokens[0].lexeme);
		for(let i = 1; i < tokens.length; ++i){
			...
			if(an_flag){
				...
				if(operators.includes(tokens[i].type)){
					...
					++log_tos;
					// error_prompt("Pushed into Log-Stack: " + tokens[i].lexeme);
					an_flag = false;
				}
				// check if its literal or variable
					...
					++bool_tos;
					// error_prompt("Pushed into Boolean-Stack: " + tokens[i].lexeme);
			...
			if(log_tos !== -1){
				while(log_stack[log_tos].operands === log_stack[log_tos].max){
					...
					--bool_tos;
					// error_prompt("Popped from Boolean-Stack: " + popped);
					...
					--log_tos;
					// error_prompt("Popped from Log-Stack: " + popped.operator);
					bool_stack.push(DUMMY); // push result of the operations (DUMMY in this case)
					++bool_tos;
					// error_prompt("Pushed into Log-Stack: result");
					if(log_tos === -1) break;
			...
			// otherwise, throw error
				...
				error_prompt("Insufficient Operands for " + log_stack[log_tos].operator);
				log_stack = [];
				log_tos = -1;
				bool_stack = [];
				bool_tos = -1;
		...
		if(log_stack.length === 1 && bool_stack.length === 0){
			error_prompt("Expected Literal/Variable/Operator after " + log_stack[log_tos].operator);
			log_stack = [];
			log_tos = -1;
			bool_stack = [];
			bool_tos = -1;
	...
	// check if only literal or variable
		...
		// error_prompt("Detected Literal/ Variable");
		return {status: FINISH, next: 1}
	}

	// otherwise, ret pass
	else{
		error_prompt("Invalid start of " + tokens[0].lexeme)
		return {status: ERROR, next: 0};
	}
}